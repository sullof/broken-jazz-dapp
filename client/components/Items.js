...
class Items extends Base {
  ...
  renderItems() {
    ...
    const filteredTokens = _.filter(this.Store.tokens, (e) =>
      id ? e.id === id : filter === "all" ? true : e[filter]
    );
    ...
    for (let token of filteredTokens) {
      allCols.push(
        <Item
          ...
          klass={
            (id ? "largevideo" : "smallvideo") +
            (token.unclaimed ? " unclaimed" : "")
          }
        />
      );
    }
        ...
        <Container>
          {allCols.length ? (
            ...
            <div className={"noTokens m0Auto"}>
              {filter === "unclaimed"
                ? "All the tokens have been claimed :-)"
                : filter === "claimed"
                ? "All the claimed tokens have been minted :-)"
                : filter === "minted"
    ...
    if (/^\d+$/.test(filter)) {
      ...
      switch (filter) {
        case "minted":
          intro = (
            <div>
              <b className={"noMobile"}>
                Minted NFTs are standard ERC-721 tokens on the blockchain.
                Sell/buy them on{" "}
                <Ab
        ...
        case "claimed":
          intro = (
            <div>
              <b className={"noMobile"}>
                Claimed items are just videos on IPFS; if you claimed some, mint
                your NFT now.
              </b>
            </div>
          );
        ...
        case "unclaimed":
          intro = (
            <div>
              <b className={"noMobile"}>
                If you own a Broken Jazz CD, use the 6-chars serial to claim
                yours. If not, you can buy a CD on{" "}
                <Ab link="https://amazon.com/dp/B08YCV1QL7" label="Amazon" />.
              </b>
            </div>
          );
          break;
        default:
          intro = (
            <div>
              <b className={"noMobile"}>
                To get a BKJZ NFTs, first buy a CD on{" "}
                <Ab link="https://amazon.com/dp/B08YCV1QL7" label="Amazon" />.
              </b>
            </div>
          );
  ...
  render() {
    if (this.Store.networkNotSupported) {
      return (
        <Container style={{ marginTop: 100 }}>
          <div className={"noTokens m0Auto"}>
            <p>
              Please, connect Metamask to the Polygon (ex-MATIC) Network :-)
            </p>
    ...
    return ready ? (
      ...
      <LoadingApp
        className="Lato"
        message={
          this.state.loadingMessage || "Loading the data from the blockchain..."
        }
      />
    );
  }
