...
export default class Menu extends Base {
  constructor(props) {
    ...
    this.bindMany(["expandAddress", "checkPathname"]);
  }
    ...
    return (
      <Navbar fixed="top" bg="dark" expand="lg" className={"roboto"}>
        ...
        <Navbar.Collapse id="responsive-navbar-nav">
          ...
          <Link to="/items/unclaimed">
            <i className="fas fa-baby-carriage" />{" "}
            {getTitle("unclaimed", "Available")}
          </Link>

          <Link to="/items/claimed">
            <i className="fas fa-chart-pie" /> {getTitle("claimed")}
          </Link>

          <Link to="/items/minted">
            <i className="fas fa-bowling-ball" /> {getTitle("minted")}
          </Link>

          <Link to="/items/all">
            <i className="fas fa-globe-americas" /> {getTitle("all")}
          </Link>

          <Link to="/items/yours">
            <i className="fas fa-cannabis" /> {getTitle("yours")}
          </Link>

          <div className={"splitter2"}>
            <br />
          </div>
        </Navbar.Collapse>
        ...
        <Navbar.Collapse className="justify-content-end">
          ...
          {this.Store.signedInAddress ? (
            ...
            <Button onClick={this.props.connect} variant="primary">
              Connect your wallet
            </Button>
          )}
